...
class NIDAQmx:
    ...
    def getAIVoltageRange(cls, device='Dev1'):
        ret = niSystem.local().devices[device].ai_voltage_rngs  # self.devices[device].ai_voltage_rngs
        return [tuple(ret[6:8])]

    @classmethod
    def getAOVoltageRange(cls, device='Dev1'):
        ret = niSystem.local().devices[device].ao_voltage_rngs  # self.devices[device].ao_voltage_rngs
